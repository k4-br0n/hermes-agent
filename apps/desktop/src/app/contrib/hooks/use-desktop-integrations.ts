...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    return registerProfileSwitchAnchorCapture(() => {
        ...
        return { connectionId: activeConnectionId, profile: activeProfile }
      })
  }, [activeConnectionId, activeProfile, sessions, visibleStoredSessionId])
  ...
  useEffect(() => {
    ...
    if (intent.connectionId !== activeConnectionId) {
      // Connection identity is authoritative even while the target profile is
      // still activating. Cancel immediately so an intent can never survive
      // onto a same-named profile on another backend.
      clearProfileSwitchRestoreIntent(intent.sequence)
    ...
    void (async () => {
      ...
      const refreshedSessions = $sessions.get()
      clearProfileSwitchRestoreIntent(intent.sequence)

      if (last && sessionBelongsToProfile(refreshedSessions, last, activeProfile)) {
