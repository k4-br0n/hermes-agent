...
describe('profile switch behavior preference', () => {
  ...
  it('replaces an older restore intent with the latest profile and connection scope', () => {
    registerProfileSwitchAnchorCapture(() => ({ connectionId: 'remote-a', profile: 'alpha' }))
    ...
    expect(latest).toMatchObject({ connectionId: 'remote-a', profile: 'gamma' })
